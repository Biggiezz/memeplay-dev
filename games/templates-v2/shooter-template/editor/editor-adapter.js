...
/**
 * Editor Adapter for Shooter Template
 * Handles save/load with localStorage and Supabase sync
 */
export class ShooterEditorAdapter extends BaseAdapter {
    constructor(options = {}) {
        super(options);
    ...
    async save(forcedGameId = null) {
        ...
        try {
            const storageKey = `${SHOOTER_STORAGE_PREFIX}${gameId}`;
    ...
    isDirty() {
        ...
        const storageKey = `${SHOOTER_STORAGE_PREFIX}${this.lastSavedGameId}`;
    ...
    async syncToSupabase(gameId, config) {
        try {
            ...
            const templateUrl = `${baseUrl}/games/templates-v2/shooter-template/index.html?game=${gameId}`;
            const publicUrl = buildPublicLinkUrl(gameId, { forceProduction: true });
            
            const creatorKey = 'shooter_creator_id';
            ...
            const payload = {
                ...
                p_title: config.storyText || 'Shooter Game',
                p_map_color: config.mapColor || '#1a1a2e',
                p_map_index: 0,
