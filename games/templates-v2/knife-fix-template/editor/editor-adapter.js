...
const TEMPLATE_ID = 'knife-fix-template';

/**
 * Editor Adapter for Knife Fix Template
 * Handles save/load with localStorage and Supabase sync
 */
export class KnifeFixEditorAdapter extends BaseAdapter {
    constructor(options = {}) {
        super(options);
    ...
    async save(forcedGameId = null) {
        ...
        try {
            const storageKey = `${KNIFE_FIX_STORAGE_PREFIX}${gameId}`;
    ...
    isDirty() {
        ...
        const storageKey = `${KNIFE_FIX_STORAGE_PREFIX}${this.lastSavedGameId}`;
    ...
    async syncToSupabase(gameId, config) {
        try {
            ...
            const templateUrl = `${baseUrl}/games/templates-v2/knife-fix-template/index.html?game=${gameId}`;
            const publicUrl = buildPublicLinkUrl(gameId, { forceProduction: true });
            
            const creatorKey = 'knife-fix_creator_id';
            ...
            const payload = {
                ...
                p_title: config.storyText || 'Knife Fix Game',
