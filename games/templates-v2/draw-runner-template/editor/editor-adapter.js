...
const TEMPLATE_ID = 'draw-runner-template';

/**
 * Editor Adapter for Draw Runner Template
 * Handles save/load with localStorage and Supabase sync
 */
export class DrawRunnerEditorAdapter extends BaseAdapter {
    constructor(options = {}) {
        super(options);
    ...
    async save(forcedGameId = null) {
        ...
        try {
            const storageKey = `${DRAW_RUNNER_STORAGE_PREFIX}${gameId}`;
    ...
    isDirty() {
        ...
        const storageKey = `${DRAW_RUNNER_STORAGE_PREFIX}${this.lastSavedGameId}`;
    ...
    async syncToSupabase(gameId, config) {
        try {
            ...
            const templateUrl = `${baseUrl}/games/templates-v2/draw-runner-template/index.html?game=${gameId}`;
            const publicUrl = buildPublicLinkUrl(gameId, { forceProduction: true });
            
            const creatorKey = 'draw-runner_creator_id';
            ...
            const payload = {
                ...
                p_title: config.storyText || 'Draw Runner Game',
                p_map_color: '#5c4033', // Fixed ground color
                p_map_index: 0,
