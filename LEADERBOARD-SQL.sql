-- ============================================
-- LEADERBOARD SCORE SYSTEM - AUTO FOR ALL GAMES
-- ============================================
-- Pattern giống LIKE/COMMENT: tự động hoạt động cho mọi game
-- Creator thêm game mới → leaderboard tự động có, không cần config
--
-- ⚠️ QUAN TRỌNG: Chỉ cần chạy 1 LẦN DUY NHẤT khi setup!
-- Sau đó tự động hoạt động cho mọi game mới.
--
-- Nếu chưa setup Like/Comment, dùng file SETUP-SUPABASE-ONCE.sql thay vì file này.
...
-- Logic: Upsert best score (chỉ update nếu score mới > best score hiện tại)
...
BEGIN
  ...
  -- Upsert: luôn insert/update, nhưng chỉ set is_new_best nếu score mới > best score
...
-- Return: top players + user entry với rank
...
BEGIN
  ...
  FROM (
    SELECT 
      ...
      -- DENSE_RANK: cùng score → cùng rank, nhưng vẫn có gap (1,1,3,3,5)
      -- Hoặc dùng COUNT(*) để tính rank đúng (số players có score >= current)
      (
  ...
  -- Return result
  -- Note: Nếu user_entry null, return null thay vì 'null' string
  RETURN json_build_object(
...
-- BƯỚC 6: Tạo policies (Public read + write)
...
-- ✅ XONG! Giờ test lại
-- Test query:
-- SELECT submit_game_score('user123', 'pixel-space-shooter', 1000, 3);
-- SELECT get_game_leaderboard_with_user('pixel-space-shooter', 'user123', 10);
