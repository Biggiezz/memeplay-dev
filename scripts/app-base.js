...
// ✅ Only difference is getUserId() - detects Base App user (FID) instead of wallet/local
...
// ✅ BASE APP: Override getUserId() - Priority: Base App (FID) > Wallet > Local
// This is the only difference compared to app-v3.js
function getUserId() {
  // Priority 1: Base App user (FID) - if running inside Base App
...
function initStatsOverlay() {
  ...
  async function updateStatsOverlay() {
    ...
    try {
      ...
      // 2. Query referral rewards from base_referral_rewards table (as referrer)
...
async function processReferralJoin(referralCode) {
  ...
  try {
    // Call RPC: base_process_referral
    const { data, error } = await supabase.rpc('base_process_referral', {
...
function detectReferralCode() {
  // Method 1: Check URL parameter (Base App uses ?ref=)
