...
// ==========================================
// Base App auto-connects wallet via window.ethereum
// This code runs immediately when module loads to init Base App

(function initBaseApp() {
  // Check if running inside Base App
  const isBaseApp = window.ethereum?.isBase || window.parent !== window;
  if (!isBaseApp) {
    console.warn('[Base App] Not running in Base App - running in browser');
  ...
  if (cached) {
    try {
      ...
      if (Array.isArray(games) && games.length > 0 && age < GAME_LIST_CACHE_TTL) {
        // ✅ Sort games from cache by likes DESC (trừ Pet Avatar)
        // Cache có thể chứa games chưa được sort đúng
        ...
        return [petAvatarGame, ...sortedGames]
      } else {
...
// ✅ BASE APP: Override getUserId() - Priority: Base App Wallet > Local
...
function getUserId() {
  ...
  if (isBaseApp) {
    ...
    if (wallet) return wallet; // Format: "0x..." (wallet address)
  }
...
if (document.readyState === 'loading') {
  ...
  // ✅ Base App: Skip referral code detection (Telegram-specific)
  // setTimeout(() => {
  //   detectReferralCode()
...
function initStatsOverlay() {
  ...
  async function updateStatsOverlay() {
    ...
    try {
      ...
      // Total = playtime (database) - only for display
